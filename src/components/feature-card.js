  ...
  return (
    <Box sx={styles.card}>
      ...
      <Box sx={styles.wrapper}>
        ...
        <Flex>
          {phone ? (
            <Link sx={styles.wrapper.whatsapp} href={`tel:${phone}`}>
              <FaPhone size={20} />
            </Link>
          ) : (
            <Text sx={styles.wrapper.noText}></Text>
          )}
          {whatsapp ? (
            <Link
              sx={styles.wrapper.whatsapp}
              href={`https://api.whatsapp.com/send?phone=${whatsapp}&text=Hola!`}
            >
              <FaWhatsapp size={30} />
            </Link>
          ) : (
...
const styles = {
  ...
  img: {
    width: ["80px", null, null, null, "75px", "90px", null, "100px"],
    height: "auto",
    flexShrink: 0,
